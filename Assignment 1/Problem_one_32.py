...
class Vigenere:
    ...
    def encrypt(self, key=None, write=True):
        """
        ...
        for i in range(len(input_array)):
            output_array.append( ( input_array[i] + key_array[i % len(key_array)] ) % self.SIZE )
        if write == False:
    ...
    def decrypt(self):
        ...
        output_array = []
        for i in range(len(input_array)):
            output_array.append( ( input_array[i] - key_array[i % len(key_array)] + self.SIZE ) % self.SIZE )
        return ''.join([self.int_to_char(i) for i in output_array])
